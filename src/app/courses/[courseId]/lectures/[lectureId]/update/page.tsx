"use client"
import { updateLecture } from '@/actions/lectureAction/updateLecture';
...
const getLecture = async (courseId: string, lectureId: string) => {
  const response = await fetch(`${API_URL}/api/courses/${courseId}/lectures/${lectureId}`)
  return response.json();
}

const LectureUpdatepage = async ({ params: { courseId, lectureId } }: Props) => {
  ...
  return (
    <div>
      <h1 className='text-center text-4xl my-10'>강의 수정</h1>
      <form
        ...
        <input
          ...
          className='outline-none border-gray-200 border h-16 px-3 rounded-sm focus:border-purple-400 focus:pt-3 focus:placeholder:text-xs focus:placeholder:-translate-y-5 placeholder:transition-all transition-all' />
        <CustomeButton btnType='submit' title='수정하기' containerStyles='bg-purple-600 text-white px-4 py-3 rounded-xl hover:bg-purple-500 transition-all ease-in-out disabled:bg-purple-300' />
      </form>
    </div>
  )
}
